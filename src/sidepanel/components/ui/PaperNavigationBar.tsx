    ...
    <div class="card mb-4 bg-gray-50 animate-slide-in-up">
      <div class="flex items-center gap-2 sm:gap-3">
        ...
        <select
          ...
          class="flex-1 min-w-0 px-2 sm:px-3 py-2 border border-gray-300 rounded-lg text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 truncate"
          style={{ transition: 'all var(--duration-normal) var(--ease-out)' }}
        >
          {papers.map((paper, idx) => (
            <option key={paper.id} value={idx} class="truncate">
              {paper.title}
            </option>
          ))}
        </select>
        ...
        {/* Paper Counter */}
        <span class="text-xs sm:text-sm text-gray-600 whitespace-nowrap animate-fade-in flex-shrink-0 hidden xs:inline">
